...
#define FORP_HIGHLIGHT_END              "<div style='position: absolute; top: 0px; right: 0px; left: 0px; height: 14px; background: #555; color: #eee; padding: 2px 5px; font-family: Consolas, monaco, monospace; font-size: 10px; text-align: right'>%s [%.03f ms, %ld b, level %d]</div></div>"
...
static void (*old_execute_internal)(zend_execute_data *current_execute_data, struct _zend_fcall_info *fci, int return_value_used TSRMLS_DC);
static void forp_execute_internal(zend_execute_data *current_execute_data, struct _zend_fcall_info *fci, int return_value_used TSRMLS_DC);
#endif
